...
do
	...
	#then
		...
		antsApplyTransforms -d 3 -v --output-data-type short \
			...
			--reference-image Finaltemplate0.nii.gz \
			--interpolation GenericLabel \
			--input ${i}_dkt_with_latvent_rings.nii.gz \
			--output ${i}_dkt_with_latvent_rings_to_template.nii.gz
		antsApplyTransforms -d 3 -v --output-data-type short \
			--transform Final${i}1Warp.nii.gz \
			--transform Final${i}0GenericAffine.mat \
			--reference-image Finaltemplate0.nii.gz \
		...
		antsApplyTransforms -d 3 -v --output-data-type float \
			...
			--output ${i}_t2_laplacian1_to_template.nii.gz 
	#fi
	DKTIMAGES="$DKTIMAGES ${i}_dkt_to_template.nii.gz"
	DKTWSKULLIMAGES="$DKTWSKULLIMAGES ${i}_dkt_with_skull_label_to_template.nii.gz"
	DKTWLATVENTIMAGES="$DKTWLATVENTIMAGES ${i}_dkt_with_latvent_rings_to_template.nii.gz"
	...
	mri_binarize --i ${i}_dkt.nii.gz --o ${i}_dkt_gm.nii.gz --min 1000 --noverbose
	mri_binarize --i ${i}_dkt.nii.gz --o ${i}_dkt_gm_lh.nii.gz --min 1000 --max 1036 --noverbose
	mri_binarize --i ${i}_dkt.nii.gz --o ${i}_dkt_gm_rh.nii.gz --min 2000 --max 2036 --noverbose
...
ImageMath 3 FinaltemplateDKTWithLatVentRingsMajority.nii.gz MajorityVoting $DKTWLATVENTIMAGES
...
ImageMath 3 FinaltemplateBrainProbRegistrationMaskDilated.nii.gz MD FinaltemplateBrainProbRegistrationMask.nii.gz 10
...
mri_binarize --i FinaltemplateDKTMajority.nii.gz --o FinaltemplateDKTMajorityPericalcarine.nii.gz --match 1021 2021 --dilate 10
mri_binarize --i FinaltemplateDKTMajority.nii.gz --o FinaltemplateDKTMajorityLatVent.nii.gz --match 4 43 31 63 --dilate 1
fslmaths FinaltemplateDKTMajorityPericalcarine.nii.gz -mas FinaltemplateDKTMajorityLatVent.nii.gz -mul -1 -add 1 -mul FinaltemplateBrainProbRegistrationMaskDilated FinaltemplateBrainProbRegistrationMaskDilatedNoLatVent
